...
import { templateCommandSchema, templateDataSchema } from './template-api';
...
export const apiRequestSchema = z.discriminatedUnion('action', [
  ...
  ...templateCommandSchema.options.map((command) => command.extend(accessFields)),
  ...
  accessCommandSchema.options[4].extend(accessFields),
  z.strictObject({
    ...requestFields,
...
export const templateResponseSchema = z.union([
  z.strictObject({
    ok: z.literal(true),
    requestId: z.uuid(),
