...
Для текущего staging проект уже создан, `.clasp.json` настроен, код загружен. Владелец подтвердил успешный запуск `setupStaging` и доступность таблицы и папки. Следующий шаг — [публикация web app](#публикация-staging-web-app); повторять создание ресурсов не нужно.
