...
Схема D-01 и перенос D-02a подтверждены настоящими повторными запусками владельца. Следующий шаг — [activateStagingSheetsAuth и проверка доступа](sheets-auth-staging.md).

Актуальное состояние на 3 сентября: опубликован web app **версии 5** с переключением проверки доступа на Sheets. Редактор содержит диагностику, настройку схемы, перенос и `activateStagingSheetsAuth`. До запуска активации действует прежний реестр. [Проверки выпуска](evidence/2026-09-03-sheets-auth-release.json). Сама загрузка исходников в редактор не меняет опубликованную версию API; для этого отдельно создаётся и выбирается снимок версии.
