...
Текущий серверный выпуск — **версия 9**, опубликована 3 сентября в 14:00:34 UTC из `5a91e3d`: [приглашения и управление правами](access-management.md), с проверенной совместимостью записи с ES2020 без `Array.at`. Код редактора и снимок проверены, manifest и URL сохранены. [Проверки выпуска](evidence/2026-09-03-access-compatibility-release.json). Используется уже подготовленная схема 2; миграция, импорт и изменение Script Properties не требуются. После выпуска владелец [подтвердил работу модуля](evidence/2026-09-03-access-management-owner.json).

Предыдущая версия **7** опубликована в 12:53:30 UTC из `d9441f7`: [журнал операций и аудит](operation-journal.md). Владелец подтвердил подготовку и повтор записи. [Проверки выпуска версии 7](evidence/2026-09-03-operation-journal-release.json). После появления новых операций доступа старый сервер версии 7 не подходит для отката: он не знает их формат.
...
Предыдущий серверный выпуск — **версия 6**, опубликована 3 сентября в 12:06:00 UTC из `b85e01b`. Добавлены `admin.users.list` и `admin.health`; URL и manifest сохранены. [Раздел владельца и границы проверки](workspace-admin.md). Повторный запуск активации или миграции для того выпуска не требовался.
