...
После успешного D-01 в редактор также загружены `planStagingUsers` и `setupStagingUsers` для переноса пользователей и приглашений: [инструкция](users-import-staging.md). Настоящее выполнение этого переноса ещё ожидается.

Актуальное состояние на 3 сентября: опубликованный web app остаётся на версии 4. Редактор содержит дополнительные функции диагностики и D-01: `planStagingSchema` и `setupStagingSchema`. [Инструкция настройки схемы](schema-migrations.md). Загрузка исходников в редактор не меняет опубликованную версию API.

Для текущего staging проект уже создан, `.clasp.json` настроен, код загружен. Владелец подтвердил успешный запуск `setupStaging` и доступность таблицы и папки. Web app версии 1 опубликован и подключён к локальному Tastory; GET/POST health проверены. [Результаты подключения](staging-verification.md). Повторять создание ресурсов и deployment не нужно.
