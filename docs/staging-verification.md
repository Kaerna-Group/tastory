...
Отказ неприглашённому аккаунту, отзыв и восстановление также не выполнялись в этой записи; их отметки `false` не означают сбой. S0-04/S0-05 остаются открытыми. Для продолжения нужна [новая запись повторного входа](google-access-staging.md#1-повторный-вход-владельца) и отдельный тестовый аккаунт для остальных сценариев. Содержимое реестра привязок и число строк этим отчётом не проверяются.
