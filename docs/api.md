...
`recipes.template.get` читает применённый снимок после проверки права на рецепт. `recipes.template.apply` принимает `recipeId`, `templateId`, `expectedRecipeRevision`, `expectedRecipeTemplateRevision` и `expectedRecipeDesignRevision`; сервер независимо проверяет содержание, назначение шаблона, дизайн, право изменения и доступность источника. `recipes.design.get/save` читают и сохраняют отдельную целостную запись оформления с собственной ревизией. Повтор записи использует прежний `requestId`, а конфликт не удаляет локальный черновик. Ответы имеют kind `templateLibrary`, `template`, `recipeTemplate` или `recipeDesign`. Ошибки: `TEMPLATE_NOT_READY`, `TEMPLATE_INVALID`, `TEMPLATE_UNAVAILABLE`, `TEMPLATE_CONFLICT`, `TEMPLATE_LIMIT`; отказ по роли возвращает `ACCESS_DENIED`. [Модель и права V-02](recipe-templates.md) и [решение о долговечном дизайне](adr/0007-durable-recipe-design.md).

## Журнал операций

Три дополнительных действия требуют credential, строго пустой payload, staging и активного владельца. Повторная проверка прав и срока токена выполняется под ScriptLock. Последовательность миграций 002–009 доводит хранилище до схемы 9; миграция 007 добавляет четыре таблицы стикеров, 008 — три таблицы шаблонов, а 009 — таблицу целостных записей дизайна. Вход поддерживает версии 1–9. `admin.health` возвращает пары schemaVersion/tablesChecked от 1/6 до 9/25. Транспортное meta не меняется.
