...
Личная кулинарная тетрадь. В коде готовы модель и права рецепта, надёжное версионируемое сохранение, локальные черновики, история, резервные копии, приватные фотографии, переносимый импорт/экспорт, [стикер-паки](docs/sticker-packs.md), [шаблоны блюд и напитков](docs/recipe-templates.md) и [ограниченная работа без сети](docs/pwa-offline.md). Основа репозитория и HTTPS staging опубликованы; новые модули ещё требуют живого выпуска и проверки. Текущий статус и оставшиеся этапы указаны в [roadmap](docs/roadmap.md).
